"""Produce air quality data to 'air_quality' Kafka topic from OpenWeatherMap API."""
import os
import json
import time
import logging
import configparser
from datetime import datetime, timezone
from typing import Optional, Dict, Any

from kafka import KafkaProducer
from airquality_utils import get_air_quality_data, aggregate_summary

# -------------------- Config --------------------
KAFKA_BROKER_URL = os.getenv("KAFKA_BROKER_URL", "kafka:9092")  # <— service name
TOPIC_NAME       = os.getenv("TOPIC_NAME", "air_quality")
SLEEP_TIME       = max(5, int(os.getenv("SLEEP_TIME", "600")))
LOG_LEVEL        = os.getenv("LOG_LEVEL", "INFO")

CITY = os.getenv("CITY", "London")
LAT  = float(os.getenv("LAT", "51.5073219"))
LON  = float(os.getenv("LON", "-0.1276474"))

# -------------------- Logging --------------------
logging.basicConfig(level=LOG_LEVEL, format="%(asctime)s %(levelname)s %(message)s")
log = logging.getLogger("airquality-producer")

def _load_api_key(cfg_path: str = "airquality_service.cfg") -> Optional[str]:
    try:
        cfg = configparser.ConfigParser()
        if not cfg.read(cfg_path):
            log.error("Config file '%s' not found.", cfg_path)
            return None
        token = cfg["openweathermap_api_credential"]["access_token"]
        if not token:
            log.error("Missing 'access_token' in config.")
            return None
        return token
    except Exception as e:
        log.error("Failed to read API key: %s", e)
        return None

def build_payload(api_key: str) -> Optional[Dict[str, Any]]:
    data = get_air_quality_data(lat=LAT, lon=LON, api_key=api_key)
    if not data:
        return None
    return aggregate_summary(
        raw_json=data, 
        city=CITY,
        producer_ts=datetime.now(timezone.utc).isoformat(),
    )



def _build_key(payload: Dict[str, Any]) -> bytes:
    provider_ts = payload.get("provider_ts") or payload.get("timestamp")
    key_str = f"{payload.get('city','unknown')}|{provider_ts}"
    return key_str.encode("utf-8")

def main():
    api_key = _load_api_key()
    if not api_key:
        log.error("No API key available. Exiting.")
        return

    log.info(
        "Starting AirQuality Producer | Broker=%s | Topic=%s | City=%s lat=%s lon=%s",
        KAFKA_BROKER_URL, TOPIC_NAME, CITY, LAT, LON
    )

    producer = KafkaProducer(
        bootstrap_servers=[KAFKA_BROKER_URL],
        acks="all",
        retries=3,
        linger_ms=50,
        compression_type="gzip",
        value_serializer=lambda v: json.dumps(v, separators=(",", ":")).encode("utf-8"),
    )

    try:
        while True:
            payload = build_payload(api_key)
            if payload:
                key = _build_key(payload)
                producer.send(TOPIC_NAME, key=key, value=payload)
                log.info(
                    "sent key=%s city=%s aqi=%s pm2_5=%s pm10=%s o3=%s no2=%s",
                    key.decode(), payload.get("city"), payload.get("aqi"),
                    payload.get("pm2_5"), payload.get("pm10"),
                    payload.get("o3"), payload.get("no2"),
                )
            else:
                log.warning("skip cycle (no payload)")
            time.sleep(SLEEP_TIME)
    finally:
        try:
            producer.flush(timeout=10)
        except Exception:
            pass
        log.info("Producer closed.")

if __name__ == "__main__":
    main()
